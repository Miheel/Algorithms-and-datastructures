...
namespace cor::graph {
	...
	{
	public:
		U_Graph(Matrix &matrix, Vertices &vertices);
		~U_Graph() = default;
	...
	template<class Matrix, class Vertices, class Node>
	inline U_Graph<Matrix, Vertices, Node>::U_Graph(Matrix &matrix, Vertices &vertices)
		:matrix(matrix), vertices(vertices)
	{
	}

	template<class Matrix, class Vertices, class Node>
	void U_Graph<Matrix, Vertices, Node>::dijkstra(Node startNode, Node endNode)
	{
		this->reset();
	...
	template<class Matrix, class Vertices, class Node>
	Node U_Graph<Matrix, Vertices, Node>::minDist()
	{
		Node tempMin;
		auto min = cor::graph::DISTANCE::infinity();
	...
	template<class Matrix, class Vertices, class Node>
	bool U_Graph<Matrix, Vertices, Node>::depthFirst(Node srcNode)
	{
		this->reset();
		...
		while (!stack.empty())
		{
			...
			for (size_t i = 0; i < this->matrix[node.ID].size(); i++)
			{
				...
				if (!visited && weight != cor::graph::NOT_CONNECTED::infinity())
				{
	...
	template<class Matrix, class Vertices, class Node>
	bool U_Graph<Matrix, Vertices, Node>::breadthFirst(Node srcNode)
	{
		this->reset();
	...
	template<class Matrix, class Vertices, class Node>
	void U_Graph<Matrix, Vertices, Node>::makeSymetric()
	{
		for (auto &elem : vertices)
		{
			if (elem.visited == false)
	...
	template<class Matrix, class Vertices, class Node>
	inline void U_Graph<Matrix, Vertices, Node>::printPath(Vertices spt, Node src, Node target)
	{
		Node tempTarget = target;
		if (spt.at(tempTarget.ID).distance != cor::graph::DISTANCE::infinity() || tempTarget.ID == src.ID)
		{
			std::cout << "\nEnd ";
			//while the target node has a minimum distance 
			//print out the node ID 
			//set new target to node in spt at target ID
			while (tempTarget.distance != cor::graph::DISTANCE::infinity())
			{
	...
	template<class Matrix, class Vertices, class Node>
	void U_Graph<Matrix, Vertices, Node>::reset()
	{
		for (auto &elem : vertices)
		{
			elem.visited = false;
			elem.distance = cor::graph::DISTANCE::infinity();
		}
	}
